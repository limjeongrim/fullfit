        ...
        <div className="max-w-7xl mx-auto px-6 py-8">
          ...
          <div className="bg-white rounded-xl shadow-sm overflow-x-auto border border-blue-100">
            <table className="w-full text-sm">
              ...
              <tbody>
                {returns.length === 0 ? (
                  <tr><td colSpan={7} className="text-center py-10 text-gray-400">반품 데이터가 없습니다.</td></tr>
                ) : (
                  returns.map((r) => {
                    ...
                    return (
                      <tr key={r.id} className="border-t border-gray-100 hover:bg-blue-50 transition-colors">
                        ...
                        <td className="px-4 py-3 text-gray-500 text-xs max-w-[160px] truncate">{r.inspection_note || '—'}</td>
                              ...
                              {actions.map((a) => (
                                <button key={a.targetStatus} onClick={() => openModal(r.id, a.targetStatus)}
                                  className={`px-3 py-1 text-xs rounded-lg font-medium transition-colors ${
                                    a.targetStatus === 'RESTOCKED' ? 'bg-green-600 hover:bg-green-700 text-white' :
                                    a.targetStatus === 'DISPOSED'  ? 'bg-gray-500 hover:bg-gray-600 text-white' :
