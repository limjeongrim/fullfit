...
function ExpiryBadge({ days }) {
  if (days <= 30)
    return <span className="px-2 py-0.5 rounded-full text-xs font-semibold bg-red-100 text-red-700">만료임박</span>
  if (days <= 60)
    return <span className="px-2 py-0.5 rounded-full text-xs font-semibold bg-yellow-100 text-yellow-700">주의</span>
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                          row.storage_type === 'COLD' ? 'bg-cyan-100 text-cyan-700' : 'bg-gray-100 text-gray-600'
                        }`}>
